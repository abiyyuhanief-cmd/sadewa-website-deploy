...
export default async function AdminDashboardPage() {
  ...
  const [{ count: guimCount }, { count: ceritaPublished }, { count: ceritaDraft }] = await Promise.all([
    ...
    <div>
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-3">
        ...
        <Link
          ...
          className="rounded-[4px_20px_4px_20px] border border-paper-200 bg-paper-white p-6 hover:shadow-md"
        >
          <p className="font-display text-3xl font-semibold text-ink-500">{ceritaDraft ?? 0}</p>
