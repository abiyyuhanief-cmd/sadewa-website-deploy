      ...
      <section className="relative overflow-hidden bg-ink-900 text-paper-50">
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-20 sm:py-24">
          ...
          <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4">
            {[
              { icon: "🎓", value: guimCumulativeStats.siswa.toLocaleString("id-ID") + "+", label: "Siswa unik" },
              { icon: "👩‍🏫", value: guimCumulativeStats.guru.toLocaleString("id-ID") + "+", label: "Guru unik" },
              { icon: "🏫", value: guimCumulativeStats.sd, label: "Sekolah dasar" },
              { icon: "📍", value: guimCumulativeStats.provinsi, label: "Provinsi" },
            ].map((s) => (
              <div key={s.label} className="rounded-[4px_20px_4px_20px] bg-white/5 p-5">
                <p className="text-lg">{s.icon}</p>
                <p className="mt-2 font-display text-2xl font-semibold text-teal-300 sm:text-3xl">{s.value}</p>
