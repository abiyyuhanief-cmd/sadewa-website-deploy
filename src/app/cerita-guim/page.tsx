    ...
    <>
      <section className="relative overflow-hidden bg-ink-900 text-paper-50">
        <div className="relative mx-auto max-w-6xl px-6 py-20 sm:py-24">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-teal-300">Cerita GUIM</p>
          <h1 className="mt-3 max-w-2xl font-display text-4xl font-semibold leading-tight sm:text-5xl">
            Satu Dekade Gerakan UI Mengajar
          </h1>
          <p className="mt-6 max-w-xl text-lg text-ink-200">
            Sepuluh angkatan, satu misi: memotivasi siswa didik sekolah dasar di daerah pelosok agar
            menjadi lebih unggul dan menginspirasi Indonesia.
          </p>
          <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4">
            {[
              { value: guimCumulativeStats.siswa.toLocaleString("id-ID") + "+", label: "Siswa unik" },
              { value: guimCumulativeStats.guru.toLocaleString("id-ID") + "+", label: "Guru unik" },
              { value: guimCumulativeStats.sd, label: "Sekolah dasar" },
              { value: guimCumulativeStats.provinsi, label: "Provinsi" },
            ].map((s) => (
              <div key={s.label} className="rounded-[4px_20px_4px_20px] bg-white/5 p-5">
                <p className="font-display text-2xl font-semibold text-teal-300 sm:text-3xl">{s.value}</p>
      ...
      <section className="bg-paper-50">
        <div className="mx-auto max-w-6xl px-6 py-16">
          <p className="text-xs font-bold uppercase tracking-[0.14em] text-teal-600">10 Angkatan</p>
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {angkatanList.map((a) => (
              <Link
                ...
                className="group rounded-[4px_20px_4px_20px] border border-paper-200 bg-paper-white p-6 transition-shadow hover:shadow-lg"
              >
                <p className="text-xs font-bold uppercase tracking-wider text-teal-600">{a.nama_angkatan}</p>
      ...
      <section className="bg-paper-100">
        <div className="mx-auto max-w-6xl px-6 py-16">
          <p className="text-xs font-bold uppercase tracking-[0.14em] text-teal-600">GUIM in Data</p>
          <h2 className="mt-2 font-display text-3xl font-semibold text-ink-900 sm:text-4xl">
          ...
          <div className="mt-10 rounded-[4px_24px_4px_24px] border border-paper-200 bg-paper-white p-6 sm:p-8">
            ...
            <GuimDataChart
              ...
              metrics={[
                ...
                { label: "Pengajar", values: angkatanList.map((a) => a.jumlah_pengajar), color: "var(--teal-300)" },
              ]}
            />
          </div>
          <div className="mt-6 rounded-[4px_24px_4px_24px] border border-paper-200 bg-paper-white p-6 sm:p-8">
            ...
            <GuimDataChart
              ...
              metrics={[
                ...
                { label: "Guru", values: angkatanList.map((a) => a.jumlah_guru), color: "var(--gold-500)" },
              ]}
            />
          </div>
        </div>
